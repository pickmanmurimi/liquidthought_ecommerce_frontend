<template>
  ...
  <div class="mb-5">
    <label :for="id ? id :name" class="text-gray-500 text-sm">{{ props.text }}</label>
    <div class="relative focus-within:text-purple-500 flex text-gray-500 items-center">
      <input :id="id ? id : name" :autocomplete="autocomplete" :placeholder="placeholder"
             :type="props.type"
             v-model="content"
    ...
    <!-- error-->
    <!-- ---------------------------------------------------------------------->
    <small v-if="formError !== undefined && formError[ name ]" class="block text-red-500">
      {{ formError[name][0] }}
    </small>
    <!--    vue validate errors-->
    <template v-for="error of v$?.content.$errors" :key="error.$uid">
