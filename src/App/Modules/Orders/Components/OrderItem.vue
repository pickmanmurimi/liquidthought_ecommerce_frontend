<template>
  ...
  <div class="rounded-xl bg-white mb-10">
    <div class="md:grid grid-cols-7 md:gap-x-10 p-2 md:p-8 lg:p-0">
      ...
      <div class="col-span-3 lg:py-8 py-4">
        <div class="mb-10">
          <h3 class="text-xl">{{ orderItem.item.name }}</h3>
          <small>By <span class="text-purple-500">Nike</span></small>
          <div>
            <small v-if="orderItem.item.isAvailable" class="text-green-400">In stock</small>
      ...
      <div class="col-span-2 lg:border-l border-gray-200 text-center lg:py-8 lg:px-2 flex flex-col justify-between">
        <div>
          <p class="text-purple-500 font-bold">
            <span class="text-gray-500">{{ orderItem.item.currency }}</span>
            {{ orderItem.item.unit_price }}
          </p>
          <small class=" ">
            Total {{ orderItem.item.currency }} {{ orderItem.item.unit_price * orderItem.quantity }}
          </small>
        </div>

        <div>
          <small class="text-green-400">Free delivery</small>
          <button class="bt w-full mb-3" @click="viewProduct(orderItem.item.uuid)"> view Product</button>
        </div>
